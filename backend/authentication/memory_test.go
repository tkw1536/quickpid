...
//spellchecker:words bytes context errors testing time github quickpid backend authentication
import (
	"context"
	"errors"
	"testing"
...
func TestInMemoryBackend_UserCRUD(t *testing.T) {
	...
	b := authentication.NewInMemoryBackend()
	now := fixedNow()

	created, err := b.CreateUser(ctx, "alice", now)
	if err != nil {
		t.Fatalf("CreateUser() error = %v", err)
	}
	if created.Username != "alice" {
		t.Fatalf("CreateUser() = %+v, want username alice", created)
	}

	if _, err := b.CreateUser(ctx, "alice", now); !errors.Is(err, backend.ErrDuplicateUsername) {
...
func TestInMemoryBackend_KeyLifecycle(t *testing.T) {
	...
	if _, err := b.CreateUser(ctx, "alice", now); err != nil {
		t.Fatalf("CreateUser() error = %v", err)
	}

	expires := "2027-01-01T00:00:00Z"
	created, err := b.CreateKey(ctx, "alice", "key-1", []byte("hash-1"), api.IssueKeyRequest{
		Comment:   "laptop",
		ExpiresAt: &expires,
	}, now)
	if err != nil {
	...
	username, err := b.LookupUserByKeyHash(ctx, []byte("hash-1"))
	if err != nil {
		t.Fatalf("LookupUserByKeyHash() error = %v", err)
	}
	if username != "alice" {
		t.Fatalf("LookupUserByKeyHash() = %q, want alice", username)
	}
	...
	if _, err := b.LookupUserByKeyHash(ctx, []byte("hash-1")); !errors.Is(err, backend.ErrInvalidKey) {
		t.Fatalf("LookupUserByKeyHash() after revoke error = %v, want ErrInvalidKey", err)
	}
}
...
func TestInMemoryBackend_NotFoundErrors(t *testing.T) {
	...
	if _, err := b.CreateKey(ctx, "missing", "key-1", []byte("hash"), api.IssueKeyRequest{Comment: "x"}, now); !errors.Is(err, backend.ErrUserNotFound) {
	...
	if _, err := b.LookupUserByKeyHash(ctx, []byte("unknown")); !errors.Is(err, backend.ErrInvalidKey) {
		t.Fatalf("LookupUserByKeyHash() error = %v, want ErrInvalidKey", err)
	}
}

func TestInMemoryBackend_KeyHashIsolation(t *testing.T) {
	ctx := context.Background()
	b := authentication.NewInMemoryBackend()
	now := fixedNow()
	...
	if _, err := b.CreateKey(ctx, "alice", "key-1", hash, api.IssueKeyRequest{Comment: "one"}, now); err != nil {
		t.Fatalf("CreateKey() error = %v", err)
	}

	hash[0] = 'X'
	if _, err := b.LookupUserByKeyHash(ctx, hash); !errors.Is(err, backend.ErrInvalidKey) {
		t.Fatalf("LookupUserByKeyHash() after mutation error = %v, want ErrInvalidKey", err)
	}
	if _, err := b.LookupUserByKeyHash(ctx, []byte("shared-hash")); err != nil {
		t.Fatalf("LookupUserByKeyHash() error = %v", err)
	}
}
...
func TestInMemoryBackend_Shutdown(t *testing.T) {
	...
	if _, err := b.CreateUser(ctx, "alice", fixedNow()); err != nil {
		t.Fatalf("CreateUser() error = %v", err)
	}
...
func TestInMemoryBackend_ListKeysSorted(t *testing.T) {
	...
	for _, id := range []string{"key-b", "key-a", "key-c"} {
		if _, err := b.CreateKey(ctx, "alice", id, []byte(id), api.IssueKeyRequest{Comment: id}, now); err != nil {
...
func TestInMemoryBackend_ListUsers(t *testing.T) {
	...
	for _, username := range []string{"carol", "alice", "bob"} {
		if _, err := b.CreateUser(ctx, username, now); err != nil {
...
func TestInMemoryBackend_UpdateKeyExpiresAt(t *testing.T) {
	...
	if _, err := b.CreateKey(ctx, "alice", "key-1", []byte("hash"), api.IssueKeyRequest{Comment: "x"}, now); err != nil {
		t.Fatalf("CreateKey() error = %v", err)
	}
