...
func (s *Service) GetMount(ctx context.Context, baseURI api.ValidBaseURI) (*api.MountResponse, error) {
	...
	if errors.Is(err, backend.ErrMountNotFound) {
		return nil, api.WithErrorString(fmt.Errorf("store.GetMount: %w", err), api.MountNotFound)
	}
	if err != nil {
		return nil, api.WithErrorString(fmt.Errorf("store.GetMount: %w", err), api.DatabaseError)
	}
...
func (s *Service) ListMounts(ctx context.Context, caller *api.ValidUserInfo, params api.ListMountsParams) (*api.PaginatedMountsResponse, error) {
	...
	if err != nil {
		return nil, api.WithErrorString(fmt.Errorf("store.ListMounts: %w", err), api.DatabaseError)
	}
	return out, nil
}
...
func (s *Service) ListNamespaceMounts(ctx context.Context, caller *api.ValidUserInfo, namespace api.ValidNamespaceID, params api.ListNamespaceMountsParams) (*api.PaginatedBaseURIResponse, error) {
	...
	if errors.Is(err, backend.ErrNamespaceNotFound) {
		return nil, api.WithErrorString(fmt.Errorf("store.ListNamespaceMounts: %w", err), api.NamespaceNotFound)
	}
	if err != nil {
		return nil, api.WithErrorString(fmt.Errorf("store.ListNamespaceMounts: %w", err), api.DatabaseError)
	}
	return out, nil
}
...
func (s *Service) SetMount(ctx context.Context, caller *api.ValidUserInfo, baseURI api.ValidBaseURI, req api.ValidMountUpsertRequest) (*api.MountResponse, error) {
	...
	if err := s.store.SetMount(ctx, baseURI, req.Namespace); err != nil {
		if errors.Is(err, backend.ErrNamespaceNotFound) {
			return nil, api.WithErrorString(fmt.Errorf("store.SetMount: %w", err), api.NamespaceNotFound)
		}
		return nil, api.WithErrorString(fmt.Errorf("store.SetMount: %w", err), api.DatabaseError)
	}
...
func (s *Service) DeleteMount(ctx context.Context, caller *api.ValidUserInfo, baseURI api.ValidBaseURI) error {
	...
	if err := s.store.DeleteMount(ctx, baseURI); err != nil {
		if errors.Is(err, backend.ErrMountNotFound) {
			return api.WithErrorString(fmt.Errorf("store.DeleteMount: %w", err), api.MountNotFound)
		}
		return api.WithErrorString(fmt.Errorf("store.DeleteMount: %w", err), api.DatabaseError)
	}
	return nil
}
